...
When you add an endpoint, it is validated in the backend before it's added. If the connection fails, an error is shown immediately in the pop-up instead of adding a broken endpoint.

Common causes:

- The API key is invalid, expired, or was pasted with extra whitespace
- The wrong provider was selected for the key
- The firewall does not allow a connection to this provider (contact your server admin)

Double-check the provider and API key, then try adding the endpoint again.